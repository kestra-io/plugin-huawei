...
- `Query`/`Scan` read a single response page and do **not** paginate across `LastEvaluatedKey`; `warnIfTruncated` logs an INFO message whenever the response is truncated so large result sets aren't silently cut short — bounded `limit` (`@Min(1) @Max(1000)`, default 100) caps how much is read per page

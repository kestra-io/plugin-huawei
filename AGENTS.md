...
- ⚠️ **The supplement-data `start_date`/`end_date` format is unverified.** The SDK types them as `String` on `CreateFactorySupplementDataInstanceRequestBody` while the read side (`SupplementDataRespRows`) returns them as epoch milliseconds, and no format is documented. `StartJobRun` defaults to `yyyy-MM-dd` and passes user values through verbatim so a flow can try another form (e.g. `yyyy-MM-dd HH:mm:ss`) without a plugin rebuild; the `DLF.3051` hint on create points there first. Body keys are snake_case (`name`, `job_name`, `start_date`, `end_date`, `parallel`, `is_day_granularity`, `is_stop_when_fail`), same convention as the read side

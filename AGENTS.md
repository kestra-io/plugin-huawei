...
- `io.kestra.plugin.huawei.dataarts.StartJobRun` — Triggers an on-demand DataArts Factory (DLF) job run via the `run-immediate` route ("Executing a Job Immediately"), **not** `/start` (which only toggles a schedule and rejects run-once jobs with `DLF.3051`); resolves the new instance by querying the instance list (run-immediate returns no usable instance ID); optionally polls until terminal state
...
- `io.kestra.plugin.huawei.dataarts.StopJobRun` — Stops a running DataArts Factory job run instance; optionally polls until `manual-stop` is confirmed. ⚠️ **Currently non-functional** — see the stop-route note in Local rules
...
- ⚠️ **`StartJobRun` uses `run-immediate`, not `/start`.** DataArts Factory has two distinct job-trigger routes: `POST /v2/{pid}/factory/jobs/{name}/start` ("Starting a Job") only turns on a job's *schedule* and returns `DLF.3051` ("The request parameter is invalid") on a run-once job — there is no schedule to start; `POST /v2/{pid}/factory/jobs/{name}/run-immediate` ("Executing a Job Immediately") performs a single on-demand run producing one instance, which is what the task's resolve-new-instance + `wait`/poll contract needs. Both routes exist on a live `dayu.tr-west-1.myhuaweicloud.com` (unauthenticated probe → `APIGW.0301`); the plugin originally called `/start` and every run-once job failed with `DLF.3051`. The schedule-only `start_date` field was dropped along with the endpoint switch (it has no meaning for run-immediate). The WireMock `startPath()` helper resolves to the run-immediate route, and a dedicated test asserts `/start` is never called. ⚠️ **`run-immediate` is nevertheless non-functional on `tr-west-1` (T-Systems EU-sovereign).** An *authenticated* run-immediate call there rejects every request body with `DLF.3051 "The request parameter is invalid.  "` — verified live for both a run-once job and a periodically-scheduled one, with empty / `{"jobParams":[]}` / `{"jobParams":[],"useExecutionUser":false}` bodies, and with the `workspace` header present (omitting it → `APIGW.0106 "Invalid header parameter: workspace, required"`, an APIGW-group rule applied across `/v2/{pid}/factory/*`). `run-immediate` is **absent from the documented V2 API family** (only supplement-data/PatchData, tags, alarm, rerun-instance, release-task appear); the identical `DLF.3051` across jobs/bodies/schedules is consistent with a generic factory-group fallthrough, not a fixable body. GET `instances/detail` on the same family works (so signing/transport/workspace are correct), and the console's own trigger uses the session-authed `/v1.0/{ws}/pipelines/run-pipelines` route, which 404s (`APIGW.0101`) on the public AK/SK gateway. Conclusion: no public AK/SK job-trigger route exists on that sovereign gateway — `StartJobRun` is documented as unsupported there; standard `.com` regions are expected to accept the call (unverified against a live standard region as of this writing). Do not re-probe run-immediate body shapes on tr-west-1 expecting a fix
...
- ⚠️ **`StopJobRun` is currently non-functional**: no published stop-instance route has been found. `DataArtsService.stopInstance` is deliberately left on the dead `/v1/` path rather than guessed into a v2 shape, so the failure stays visible. All of `POST|PUT /v2/{pid}/factory/jobs/{job}/instances/{id}/stop`, `POST /v2/{pid}/factory/jobs/{job}/stop`, `POST /v2/{pid}/factory/jobs/{job}/instances/stop` and `POST /v2/{pid}/factory/jobs/instances/{id}/stop` return `APIGW.0101`, and `DataArtsStudioMeta` declares no factory-job stop route (only `instances/detail`, `instances/retry`, `rename`, `tags`, plus an unrelated `supplement-data/{name}/stop`). Resolving it needs Huawei's DLF API reference or an authenticated probe against a running instance

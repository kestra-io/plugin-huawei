...
- ⚠️ **The job-run instance ID is a `Long` and is not visible anywhere in the console.** `JobInstance.instance_id` is numeric; the `instanceId=<uuid>` query parameter in DataArts console URLs is the **DataArts Studio service instance** (SDK `InstanceDetailDTO.id`, a `String` UUID with VPC/flavor/nodes), a completely unrelated entity. The Job Monitoring grid exposes only `job_instance_name`, not the numeric ID, and it has no column for it. So the only ways to obtain a valid `instanceId` are `StartJobRun`'s output or `GetJobRun` with `instanceId` omitted (resolves the latest run). Passing the console UUID yields a bare `JsonParseException: Unrecognized token` from Kestra's property coercion, with no mention of the property or expected type

...
- `io.kestra.plugin.huawei.koocli.KooCLI` — Runs arbitrary `hcloud` CLI commands in a container (default `ubuntu:22.04`); injects AK/SK, region, and security token as env vars (`HUAWEICLOUD_SDK_AK`, `HUAWEICLOUD_SDK_SK`, `HUAWEICLOUD_SDK_REGION`, `HUAWEICLOUD_SDK_SECURITY_TOKEN`), then writes a `default` profile via a guarded `hcloud configure set --cli-profile=default` bootstrap step that references those env vars **by shell name** (e.g. `--cli-access-key="$HUAWEICLOUD_SDK_AK"`) so secrets never appear on argv or in Kestra logs — required because KooCLI does not read region (or any credential) from an environment variable, only from CLI flags or a saved profile; no global output format property (KooCLI only supports `--cli-output` per command); prepends two guarded bootstrap steps: (1) `command -v curl || apt-get install -y curl ca-certificates tar`; (2) `command -v hcloud || curl ... | bash -s -- -y` using an install script URL resolved by `resolveInstallScriptUrl(...)` in 3 tiers: (1) `installScriptUrl` (`Property<String>`, unset by default) if explicitly set — escape hatch for private/dedicated (HCS) clouds or future partitions; (2) a known sovereign region (currently `eu-west-101`) auto-selects its own `myhuaweicloud.eu` binary; (3) otherwise falls back to the international binary, which covers the full standard region catalog; both bootstrap steps auto-skip on prebuilt images; supports `temporaryCredentials` inline exchange; returns `ScriptOutput` with `vars`, `outputFiles`, `exitCode`
...
- KooCLI uses `io.kestra:script` (already a compile dependency); no new SDK; default image `ubuntu:22.04` (glibc); AK/SK, region, and security token injected as env vars (`HUAWEICLOUD_SDK_AK`, `HUAWEICLOUD_SDK_SK`, `HUAWEICLOUD_SDK_REGION`, `HUAWEICLOUD_SDK_SECURITY_TOKEN`); KooCLI does NOT read region (or any credential) from an env var — only from CLI flags or a saved profile — so a guarded `hcloud configure set --cli-profile=default` bootstrap step writes a `default` profile referencing those env vars **by shell name** (e.g. `--cli-region="$HUAWEICLOUD_SDK_REGION"`), so the actual secret values are only expanded by the shell inside the container and never appear on argv or in Kestra logs; `--cli-output` is not a valid global param, KooCLI output format is per-command only via `--cli-output=json|table|tsv`; prepends two guarded bootstrap steps: (1) `command -v curl || apt-get install -y curl ca-certificates tar`; (2) `command -v hcloud || curl ... | bash -s -- -y`, both auto-skipping on prebuilt images; each `hcloud` binary is partition-specific — it ships with a baked-in region catalog and validates `--cli-region` against it with **no runtime override** (`--cli-endpoint` does not bypass this validation) — so the install URL is resolved via a 3-tier `resolveInstallScriptUrl(...)`: (1) `installScriptUrl` property (`Property<String>`, unset/`null` by default) wins if explicitly set — escape hatch for private/dedicated (HCS) clouds or future partitions; (2) a known sovereign region in the `SOVEREIGN_INSTALL_URLS` map (currently only `eu-west-101` → `https://eu-west-101-apiexplorer-cli.obs.eu-west-101.myhuaweicloud.eu/cli/latest/hcloud_install.sh`) auto-selects its own binary; (3) otherwise falls back to `INTERNATIONAL_INSTALL_URL` (`https://ap-southeast-3-hwcloudcli.obs.ap-southeast-3.myhuaweicloud.com/cli/latest/hcloud_install.sh`), which covers the full standard region catalog (`cn-*`/`ap-*`/`la-*`/`na-*`/`af-*`/`me-*`/`ru-*`/`tr-*`/`ae-*`/`sa-*` plus `eu-west-0`) — standard regions and the EU Sovereign Cloud are both zero-config, `installScriptUrl` is only needed for HCS/dedicated/future partitions; glibc required (Alpine/musl not supported); KooCLI integration test gate: `HUAWEI_CLI_TESTS=true`; unit tests run unconditionally

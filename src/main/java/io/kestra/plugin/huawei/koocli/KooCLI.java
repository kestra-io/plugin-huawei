...
@Schema(
    ...
    description = """
        ...
        **`projectId` / `domainId`:** these connection properties (inherited from the base Huawei
        connection) are not used by this task. KooCLI does not expose a stable, version-independent
        `configure set` flag for project/domain scoping; pass a project- or domain-scoped parameter
        directly on the relevant `hcloud` command instead (consult `hcloud <service> <operation>
        --help`) if the operation requires it.
        """
)
@Plugin(
    examples = {
...
public class KooCLI extends AbstractConnection implements RunnableTask<ScriptOutput>, NamespaceFilesInterface, InputFilesInterface, OutputFilesInterface {
    ...
    public ScriptOutput run(RunContext runContext) throws Exception {
        ...
        var rContainerImage = runContext.render(this.containerImage).as(String.class).orElse(DEFAULT_IMAGE);
    ...
    // Failure is fatal: without it, user commands would run unauthenticated against a stale/empty
    // profile, masking the real error behind whatever exit code the first user command happens to return.
    private String buildConfigureCommand(AbstractConnection.HuaweiClientConfig clientConfig) {
        ...
        return String.join(" ", parts) + " || { echo \"hcloud configure set failed\" >&2; exit 1; }";
    }
}

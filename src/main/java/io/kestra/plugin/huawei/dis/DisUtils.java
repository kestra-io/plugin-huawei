...
/**
 * Static helpers for DIS endpoint resolution and project-id validation, deliberately free of
...
public final class DisUtils {
    ...
    public static String disEndpoint(String endpointOverride, String region, String endpointSuffix) {
        ...
        if (isNotBlank(region)) {
            ...
            return "https://dis." + region.trim() + "." + suffix;
        }
    ...
    /**
     * The DIS v2 APIs embed the project id in the request path (`/v2/{project_id}/...`). When the
    ...
    public static void requireProjectIdForCustomEndpoint(String rOverride, String rSuffix, String projectId) {
        ...
        if (customEndpoint && !isNotBlank(projectId)) {
            throw new IllegalArgumentException(
                "DIS requires `projectId` when a custom endpoint (`endpointOverride` or `endpointSuffix`) is used — " +

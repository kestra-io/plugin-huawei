...
/**
 * Static helpers for SMN endpoint resolution and project-id validation, deliberately free of
 * RunContext and SDK imports so they remain unit-testable without bootstrapping Micronaut.
 */
public final class SmnUtils {

    private SmnUtils() {
    }

    // Resolution order: endpointOverride → region-derived (suffix defaults to myhuaweicloud.com) → throws.
    public static String smnEndpoint(String endpointOverride, String region, String endpointSuffix) {
        ...
        if (isNotBlank(region)) {
            ...
            return "https://smn." + region.trim() + "." + suffix;
        }
    ...
    /**
     * The SMN v2 APIs embed the project id in the request path
     * (`/v2/{project_id}/notifications/topics/{topic_urn}/publish`). When the SDK resolves the endpoint
     * from its region enum it can auto-discover the project, but a custom endpoint (endpointOverride or
     * endpointSuffix — e.g. sovereign clouds) bypasses that, leaving `{project_id}` unsubstituted and the
     * gateway rejecting the call with an opaque 400. Fail fast with an actionable message instead.
     */
    public static void requireProjectIdForCustomEndpoint(String rOverride, String rSuffix, String projectId) {
        ...
        if (customEndpoint && !isNotBlank(projectId)) {
            throw new IllegalArgumentException(
                "SMN requires `projectId` when a custom endpoint (`endpointOverride` or `endpointSuffix`) is used — " +

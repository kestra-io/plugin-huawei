...
/**
 * Static helpers for EventGrid endpoint resolution and project-id validation, deliberately free of
 * RunContext and SDK imports so they remain unit-testable without bootstrapping Micronaut.
 */
public final class EventGridUtils {

    private EventGridUtils() {
    }

    // Resolution order: endpointOverride → region-derived (suffix defaults to myhuaweicloud.com) → throws.
    public static String egEndpoint(String endpointOverride, String region, String endpointSuffix) {
        ...
        if (isNotBlank(region)) {
            ...
            return "https://eg." + region.trim() + "." + suffix;
        }
    ...
    /**
     * The EventGrid v1 APIs embed the project id in the request path (`/v1/{project_id}/...`). When
    ...
    public static void requireProjectIdForCustomEndpoint(String rOverride, String rSuffix, String projectId) {
        ...
        if (customEndpoint && !isNotBlank(projectId)) {
            throw new IllegalArgumentException(
                "EventGrid requires `projectId` when a custom endpoint (`endpointOverride` or `endpointSuffix`) is used — " +

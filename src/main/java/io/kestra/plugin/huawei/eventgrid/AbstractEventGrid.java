...
@Getter
@NoArgsConstructor
public abstract class AbstractEventGrid extends AbstractConnection implements EventGridConnectionInterface {

    protected Property<String> endpointOverride;

    protected Property<String> endpointSuffix;

    protected EgClient client(RunContext runContext) throws Exception {
        ...
        var builder = EgClient.newBuilder().withCredential(creds);

        EventGridUtils.requireProjectIdForCustomEndpoint(rOverride, rSuffix, config.projectId());

        if (rOverride != null && !rOverride.isBlank()) {
            builder.withEndpoint(EventGridUtils.stripTrailingSlash(rOverride.trim()));
            ...
            if (rSuffix != null && !rSuffix.isBlank()) {
                builder.withEndpoint(EventGridUtils.egEndpoint(null, rRegion, rSuffix));
            } else {
                try {
                    builder.withRegion(EgRegion.valueOf(rRegion));
                } catch (IllegalArgumentException e) {
                    builder.withEndpoint(EventGridUtils.egEndpoint(null, rRegion, null));
                }
            }
        } else {
            throw new IllegalArgumentException(
                "EventGrid requires either `endpointOverride` or `region` to be set — " +

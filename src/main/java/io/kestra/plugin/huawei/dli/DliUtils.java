...
/**
 * Static helpers for DLI endpoint resolution and project-id validation, deliberately free of
 * RunContext and SDK imports so they remain unit-testable without bootstrapping Micronaut.
 */
public final class DliUtils {

    private DliUtils() {
    }

    // Resolution order: endpointOverride → region-derived (suffix defaults to myhuaweicloud.com) → throws.
    public static String dliEndpoint(String endpointOverride, String region, String endpointSuffix) {
        ...
        if (isNotBlank(region)) {
            ...
            return "https://dli." + region.trim() + "." + suffix;
        }
    ...
    /**
     * The DLI v1 APIs embed the project id in the request path (`/v1.0/{project_id}/...`). When the
     ...
     * `{project_id}` unsubstituted and the gateway rejecting the call with an opaque 400. Fail fast
    ...
    public static void requireProjectIdForCustomEndpoint(String rOverride, String rSuffix, String projectId) {
        ...
        if (customEndpoint && !isNotBlank(projectId)) {
            throw new IllegalArgumentException(
                "DLI requires `projectId` when a custom endpoint (`endpointOverride` or `endpointSuffix`) is used — " +
